...
- `agent_run` returns `{ runId, status: "queued" }` for a normal dispatch, or `{ runId, status: "pending_input" }` when the run is held on the recommendation card in this conversation, waiting for the person to Confirm or Skip. Tell the user: "The run is waiting for your Confirm or Skip on the recommendation card in this conversation." After `queued`, call `agent_run_get` until the run reaches `completed`, `failed`, `pending_approval`, `pending_input`, or `stopped` — a `pending_input` seen here, from a run that started `queued`, is a different, overloaded mid-execution gate, not the card (see the `chat-run-polling` skill for how to read each). After a dispatch-time `pending_input`, do not poll — the card in this conversation is what releases the run, and nothing hands control back to you when it does; call `agent_run_get` yourself later if you are still in the conversation.
...
- **Dispatch** with `agent_run { packageName, inputParams: <stringified JSON> }`. ONE primitive — internal vs external is handled server-side by the executor (`packages/agents/src/a2a-actions.ts:182-323`). Returns `{ runId, status: "queued" }` (poll with `agent_run_get`, see the `chat-run-polling` skill) or `{ runId, status: "pending_input" }` (held on the recommendation card in this conversation — do not poll; wait for the person's Confirm or Skip).
