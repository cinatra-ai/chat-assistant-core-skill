When the user wants to RUN an existing agent (not author a new one — that's the `chat-agent-authoring` skill), use the single canonical dispatch path below. After every `agent_run`, follow the `chat-run-polling` skill's reply rule: the platform's `message` is your reply, said back exactly as it is written, with nothing added and no poll.
...
- `agent_run` answers `{ runId, status, message }` — `status: "queued"` for a normal start, or `status: "pending_input"` when the run is held on the recommendation card in this conversation. The card shows a checkbox in front of every skill pill (checked means the skill applies) and one Continue — no Confirm, Adjust, or Skip; the run stays held until the person presses Continue. The reply is the same either way: say the `message` back exactly as it is written and add nothing, for the hold as much as for a started run. Do not poll a start — the run's own card re-reads its state and shows its progress there, and where your sentence and the card could disagree the card is right. Nothing hands control back to you when the person acts on the card; read the run with `agent_run_get` only if they ask how it is doing (see the `chat-run-polling` skill for the status vocabulary that read returns).
...
- **Dispatch** with `agent_run { packageName, inputParams: <stringified JSON> }`. ONE primitive — internal vs external is handled server-side by the executor (`packages/agents/src/a2a-actions.ts:182-323`). Answers `{ runId, status, message }` — say the `message` back exactly and do not poll (see the `chat-run-polling` skill). A `status: "pending_input"` means the run is held on the recommendation card in this conversation until the person presses Continue (a checkbox in front of every skill pill, unticked to leave that skill out), and the message already says so.
